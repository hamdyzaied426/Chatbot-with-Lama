...
# Initialize database tables with cascade delete
def initialize_db():
...
def get_cached_response(query):
    ...
    try:
        ...
        valid_responses = [st.session_state.cache.get(indices[0][i]) for i in range(len(similarities[0])) if similarities[0][i] > 0.60]
        if not valid_responses:
            return None
        most_common_response = Counter(valid_responses).most_common(1)[0][0]
        store_query_in_db(query, query_vector, most_common_response)
...
def ollama_generate(prompt, history, temperature=0.7):
    try:
        formatted_history = "\n".join(
...
def main_chat_interface(temperature):
    ...
    if prompt := st.chat_input("Enter your message:"):
        ...
        store_query_in_db(prompt, get_embedding(prompt), response)
